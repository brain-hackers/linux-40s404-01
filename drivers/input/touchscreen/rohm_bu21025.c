...
static int rohm_bu21025gul_write_reg(struct i2c_client *client, int data)
{
	...
	if (ret < 0) {
		dev_info(&client->dev, "_%s:master_xfer Failed!!\n", __func__);
		return ret;
	}
...
static int calibration_pointer(int *x_orig, int *y_orig)
{
	...
	*y_orig = y;
	...
	printk("data[5] = %d\n",calibration[5]);
#endif
	x = calibration[0] * (*x_orig) + calibration[1] * (*y_orig) +
	    calibration[2];
	x /= calibration[6];
	if (x < 0)
		x = 0;
	y = calibration[3] * (*x_orig) + calibration[4] * (*y_orig) +
	    calibration[5];
	y /= calibration[6];
	if (y < 0)
		y = 0;
...
static enum hrtimer_restart rohm_bu21025gul_timer_func(struct hrtimer *timer)
{
	struct rohm_bu21025gul_data *data =
		container_of(timer, struct rohm_bu21025gul_data, timer);
...
{
	struct rohm_bu21025gul_data *data =
		container_of(work, struct rohm_bu21025gul_data, work);
...
repeat:
	...
	if (read_z1 == 0) {
		...
		touch_value = ((RX_VALUE * read_x) / 4096) *
				      (((4096 * 1000) / read_z1) - 1000) -
			      ((1000 - ((read_y * 1000) / 4096)) * RY_VALUE);
	}
#ifdef DEBUG2
	...
	if ((touch == 0) || (read_z1 == 0) || (read_z1 == 4095)) {
		...
		err = rohm_bu21025gul_write_reg(client,
						(POWER_SETTING << 4) | 0x00);
		if (err) {
			dev_err(&client->dev, "work error\n");
			goto err_proc;
		}
		...
		hrtimer_start(&data->timer, ktime_set(0, 5000000),
			      HRTIMER_MODE_REL);
		//		goto repeat;
	} else {
#ifdef DEBUG2
...
#endif
		hrtimer_start(&data->timer, ktime_set(0, 10000000),
			      HRTIMER_MODE_REL);
	}
...
static long rohm_bu21025gul_ioctl(struct file *file, unsigned int cmd,
				  unsigned long arg)
{
	int ret = 0;
	int *ibuf = (int *)arg;
...
#ifdef CONFIG_PM_RUNTIME
	SET_RUNTIME_PM_OPS(bu21025_runtime_suspend, bu21025_runtime_resume,
			   NULL)
#endif /* CONFIG_PM_RUNTIME */
};
#endif

static const struct i2c_device_id rohm_bu21025gul_id[] = { { "bu21025", 0 },
							   {} };
...
static int __init rohm_bu21025gul_init(void)
{
	...
	if (device_create(ts_dev_class, NULL, MKDEV(TS_MAJOR, 0), NULL, "ts") ==
	    NULL) {
		res = -1;
	...
	if ((res = i2c_add_driver(&rohm_bu21025gul_driver)) < 0)
		goto out_destroy;
